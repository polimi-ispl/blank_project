...
from params import trained_models_root, model_name, input_shape, batch_size, epochs
...
def main():
    # Define output paths
    weights_path = os.path.join(trained_models_root, '{:s}'.format(model_name), 'model')
    history_path = os.path.join(trained_models_root, '{:s}'.format(model_name), 'history.npy')
    ...
    # Initialize model
    model = getattr(arch, model_name)(input_shape)
    model.summary()
